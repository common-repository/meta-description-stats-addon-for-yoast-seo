...
<div class="wrap netgo-admin-page-setting">
	<h2><?php _e('Meta Description Stats','');?></h2>
	<?php 
	$paged = (sanitize_text_field( $_GET['paged'] )) ? sanitize_text_field( $_GET['paged'] ) : 1;
	$post_per_page=10;
	
	if($keyword_to_search != ''){ // search description
  ...
  <table class="wp-list-table widefat fixed striped pages">
   <tr>
	...
	<td colspan="2">
	<div align="right">
	 <form method="post" action="<?php echo get_option('home');?>/wp-admin/admin.php?page=description-stats" id="posts-filter">  
		...
		<input type="search" placeholder="Meta Description" value="<?php echo $keyword_to_search;?>" name="sk" id="post-search-input">
  ...
  <table class="wp-list-table widefat fixed striped pages"> 
	...
	<thead>
	<tr>
	   <td width="50%"><strong><a class="class-to-<?php echo $classorder; ?>" href="<?php echo get_option('home');?>/wp-admin/admin.php?page=description-stats&orderby=meta_value&order=<?php echo $ancorder; ?>"> Meta Description</a></strong></td>
	...
	?>
	<tr>
	   <td width="50%"><?php echo $kword= get_post_meta( get_the_ID(), '_yoast_wpseo_metadesc', true ); ?></td>
	   <td width="30%"><?php the_title(); ?></td>
	   <?php  $post_cnt= mdsysa_get_post_count( $kword);
	...
	?>
	<tr>
	   <td colspan="4"><strong><i>No description yet.</i></strong></td>
...
<?php 
//pagination 
if (function_exists("mdsysa_pagination")) {
?>
	<tr>
	   <td colspan="4">
	    <div  style="float:right;"><?php mdsysa_pagination($the_query->max_num_pages); ?></div>
	   </td> 
	</tr>
<?php
} 
